...
from keras.layers import Dense, regularizers, Dropout, BatchNormalization, AlphaDropout, Bidirectional, LSTM, Activation
...
def training(file_path, epochs, noise_std, do_gif=False):
    ...
    for file in os.listdir(file_path):
        ...
        network_model.add(Bidirectional(LSTM(10, return_sequences=True),
                                input_shape=(5, 10)))
        network_model.add(Bidirectional(LSTM(10)))
        network_model.add(Dense(5))
        network_model.add(Activation('softmax'))
        network_model.compile(loss='categorical_crossentropy', optimizer='rmsprop')
...
if __name__ == '__main__':
    training(ENC_ANGLES_DATA_PATH, epochs=50, noise_std=0.001, do_gif=False)
